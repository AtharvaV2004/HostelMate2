...
import { Search, MapPin, Clock, Users, Filter, ShoppingBag, Star } from 'lucide-react';
import { Trip } from '@/types';
import { MOCK_TRIPS } from '@/lib/mockData';

export default function TripsList({ onTripClick }: { onTripClick: (trip: Trip) => void }) {
            ...
            <div className="flex items-center justify-between pt-4 border-t border-glass-border">
              <div className="flex items-center gap-2">
                <div className="w-8 h-8 rounded-full overflow-hidden relative">
                  <Image 
                    src={`https://picsum.photos/seed/${trip.user}/100/100`} 
                    alt={trip.user}
                    fill
                ...
                <div>
                  <div className="text-xs font-medium">{trip.user}</div>
                  <div className="flex items-center gap-0.5 text-[10px] text-amber-500">
                    <Star size={8} fill="currentColor" /> {trip.rating}
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-col items-end">
                ...
                <div className="flex gap-1">
                  {Array.from({ length: trip.total }).map((_, i) => (
                    <div 
                      key={i} 
                      className={`w-3 h-1 rounded-full ${i < (trip.total - trip.slots) ? 'emerald-gradient' : 'bg-bg-surface border border-glass-border'}`} 
                    />
                  ))}
                </div>
              </div>
